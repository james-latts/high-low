...
class TimeSeriesModel(nn.Module):
  ...
  def __init__(self, classes, input_size, window_size, hidden_layer_size, dropout_rate, device):
    ...
    self.lstm = nn.LSTM(self.input_size, self.hidden_layer_size, dropout=self.dropout_rate)
    self.linear = nn.Linear(self.window_size * self.hidden_layer_size, 15)
    self.classify = nn.Linear(15, classes)

  def init_hidden(self):
    self.hidden_cell = \
      (torch.zeros(1, self.window_size, self.hidden_layer_size).to(self.device).double(),
      torch.zeros(1, self.window_size, self.hidden_layer_size).to(self.device).double())     

  def forward(self, features):
    
    lstm_out, self.hidden_cell = self.lstm(features, self.hidden_cell)  
    linear_out = self.linear(lstm_out.view(len(features), -1))
    output = self.classify(linear_out)
    return output
...
if OPTION_TYPE in ["call", "high"]:
  #data_df["y"] = np.where(((target_df["delta"] - np.mean(target_df["delta"])) / np.std(target_df["delta"])) > 1, 1, 0) # to predict call/high options
