...
FEATURE_COLUMNS = ["Hour", "Minute", "Change", \
...
class TimeSeriesModel(nn.Module):
    def __init__(self, classes, nlen, ninp, nhid, nlayers, dropout, bi):
        ...
        self.linear = nn.Linear((1 + bi) * nhid * nlen, 256)
        self.classify = nn.Linear(256, classes)
...
data_df["Change"] = (data_df["Price"] / data_df["Last_Price"] - 1) * 100
data_df["10_sma"] = (data_df["Price"].rolling(window=10).mean() / data_df["Price"] - 1) * 100
data_df["25_sma"] = (data_df["Price"].rolling(window=25).mean() / data_df["Price"] - 1) * 100
data_df["50_sma"] = (data_df["Price"].rolling(window=50).mean() / data_df["Price"] - 1) * 100
data_df["100_sma"] = (data_df["Price"].rolling(window=100).mean() / data_df["Price"] - 1) * 100
data_df["200_sma"] = (data_df["Price"].rolling(window=200).mean() / data_df["Price"] - 1) * 100
data_df["10_ema"] = (data_df["Price"].ewm(span=10).mean() / data_df["Price"] - 1) * 100
data_df["25_ema"] = (data_df["Price"].ewm(span=25).mean() / data_df["Price"] - 1) * 100
data_df["50_ema"] = (data_df["Price"].ewm(span=50).mean() / data_df["Price"] - 1) * 100
data_df["100_ema"] = (data_df["Price"].ewm(span=100).mean() / data_df["Price"] - 1) * 100
data_df["200_ema"] = (data_df["Price"].ewm(span=200).mean() / data_df["Price"] - 1) * 100
